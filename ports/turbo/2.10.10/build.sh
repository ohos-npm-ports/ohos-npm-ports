...
# (crates/turborepo) and the TUI's libghostty-vt (via zig) compile natively on
# an OHOS host; the JS launcher is the patched packages/turbo/bin/turbo.
...
# The container execs unsigned build-time binaries as-is; the only signature
# needed is on the final published artifact below.
...
$CURL "https://ziglang.org/download/${ZIG_VERSION}/${ZIG_DIR}.tar.xz" -o zig.tar.xz
echo "958ed7d1e00d0ea76590d27666efbf7a932281b3d7ba0c6b01b0ff26498f667f  zig.tar.xz" | sha256sum -c -
tar -xJf zig.tar.xz
rm zig.tar.xz
chmod +x "${ZIG_DIR}/zig"
export PATH="$(pwd)/${ZIG_DIR}:$PATH"
# Keep zig's cache inside the working directory: /data/storage/... only
# exists on OHOS hosts, and $PWD works in both CI containers and devices.
...
# --- build ---
cargo build --release --locked -p turbo
...
# Final published artifact: sign it (needed on OHOS hosts, harmless
# elsewhere). llvm-strip drops the .codesign section the patched LLD added
# at link time, so sign after stripping. The signature section is part of
# the file content, so it survives npm/bun installs into node_modules.
...
grep -q '"@ohos-npm-ports/turbo"' package.json
