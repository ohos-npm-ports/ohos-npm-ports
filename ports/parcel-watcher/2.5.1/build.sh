...
#   a) 普通安装：optionalDependencies 增加 openharmony 槽位包 alias
#      （os 平台过滤，OHOS 安装自动拉取，其他平台跳过；拉取失败仅
#      optional 降级，不阻塞安装）
#   b) bun --compile：wrapper.js 顶部静态引用该平台包（0003），bundler
#      把它登记进内嵌模块表，运行时动态拼名 require 按 specifier 命中
#      （.node 直接作 main 不进模块表，故槽位包 main 是 index.js shim）
